...
<table width="50%" style=" border: 1px solid black;" >
    <tr>
        @if (isset($quotation->credit_days))
            ...
            <td style="width: 10%;border: 1px solid black;"> {{$newDate}} </td>

        @else
            <td style="width: 10%;border: 1px solid black;">Fecha de Emisión:</td>
            <td style="width: 10%;border: 1px solid black;">{{$datenow}} </td>
        @endif
    </tr>
</table>
...
<table width="100%" style=" border: 1px solid black;" >
    ...
    <tr>
        ...
        <td style="font-size: 12px;text-align: center;border: 1px solid black;">{{$quotation->transports->type}}--{{$quotation->transports->modelos['description']}}</td>
    ...
    @else
        @foreach ($inventories_quotations as $inventories_quotation)
            ...
            @if($inventories_quotation->retiene_iva_quotation == "0")       <!-- BASE IMPONIBLE-->
                ...
                <td style="font-size: 10px;text-align: center;border: 1px solid black;">${{number_format((($inventories_quotation->price * $inventories_quotation->amount_quotation) + ($inventories_quotation->price * $inventories_quotation->amount_quotation * 16 / 100)  + ($inventories_quotation->price * $inventories_quotation->amount_quotation * 15 / 100 * 16 / 100)) / $inventories_quotation->rate  ,2,",",".")}}</td>
                @else
